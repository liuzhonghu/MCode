...
public class SketchDataGridAdapter extends BaseAdapter {
  ...
  public SketchDataGridAdapter(Context context, List<SketchData> sketchDataList,
  ...
  @Override public SketchData getItem(int position) {
  ...
  private void showData(final ViewHolder holder, int position) {
    if (getCount() > 1 && position == getCount() - 1) {
      ...
      if (getItem(position) != null) {
        Drawable drawable =
            new BitmapDrawable(mContext.getResources(), getItem(position).thumbnailBM);
  ...
  private void bindView(View view, final ViewHolder holder, final int position) {
    ...
    holder.deleteIV.setOnClickListener(new View.OnClickListener() {
      @Override public void onClick(View v) {
        if (getCount() > 1 && onActionCallback != null) onActionCallback.onDeleteCallback(position);
      }
    });
