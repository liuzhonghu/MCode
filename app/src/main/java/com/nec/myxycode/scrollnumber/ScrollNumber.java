...
class ScrollNumber extends View {
  ...
  private int mTextSize = sp2px(120);
  ...
  public void setNumber(final int from, final int to, final int cyles, long delay) {
  ...
  private int measureHeight(int measureSpec) {
    ...
    return result + getPaddingTop() + getPaddingBottom() + dp2px(36);
  }
  ...
  private void setFromNumber(int number) {
    if (number < 0 || number > 9) throw new RuntimeException("invalidate number , should in [0,9]");
    calNum(number);
    mOffset = 0;
    invalidate();
  }
  ...
  private Runnable mScrollRunnable = new Runnable() {
    @Override public void run() {
      float x = (float) (1 - 1.0 * Math.abs(mTargetNum - mCurNum) / (mDeltaNum + 9 * mCycles));
