...
/**
 * Created by liuzhonghu on 2017/8/14.
 */
public class MultiScrollNumber extends LinearLayout {
    ...
    for (int i = mTargetNumbers.size() - 1; i >= 0; i--) {
      if (mTargetNumbers.get(i) != -1) {
        ...
        scrollNumber.setTextColor(
            ContextCompat.getColor(mContext, mTextColors[i % mTextColors.length]));
        ...
        scrollNumber.setNumber(0, mTargetNumbers.get(i), mCycles + 5 * i, i * 10);
        ...
        point.setTextColor(ContextCompat.getColor(mContext, mTextColors[i % mTextColors.length]));
  ...
  public void setNumber(int from, int to) {
    if (to < from) throw new UnsupportedOperationException("'to' value must > 'from' value");
  ...
  private final static int mCycles = 1;

  public void startScroll() {
    if (mPrimaryNumbers.size() == 0 || mTargetNumbers.size() == 0) return;
    for (int i = mTargetNumbers.size() - 1; i >= 0; i--) {
      ScrollNumber scrollNumber = new ScrollNumber(mContext);
      scrollNumber.setTextColor(
          ContextCompat.getColor(mContext, mTextColors[i % mTextColors.length]));
      scrollNumber.setTextSize(mTextSize);
      scrollNumber.setVelocity(mVelocity + (mTargetNumbers.size() - i - 1) * 5);
      if (!TextUtils.isEmpty(mFontFileName)) scrollNumber.setTextFont(mFontFileName);
      scrollNumber.setNumber(mPrimaryNumbers.get(i), mTargetNumbers.get(i),
          mCycles + (mTargetNumbers.size() - i - 1), (mTargetNumbers.size() - i - 1) * 50);
      mScrollNumbers.add(scrollNumber);
      addView(scrollNumber);
    }
    mTargetNumbers.clear();
  ...
  public void setInterpolator(Interpolator interpolator) {
    if (interpolator == null) throw new IllegalArgumentException("interpolator couldn't be null");
  ...
  public void setScrollVelocity(@IntRange(from = 0, to = 1000) int velocity) {
    ...
    for (int i = 0; i < mScrollNumbers.size(); i++) {
      mScrollNumbers.get(i).setVelocity(velocity - i * 4);
    }
  }
}
