...
/**
 * <p>
 * Description:  分段progressbar(倒计时)，可以暂停，删除上一段
 * created by liuzhonghu @17/08/04
 * </p>
 */
public class SegProgressbar extends View {

  private final int DEFAULT_WIDTH = 450;//
  private final int DEFAULT_HEIGTH = 12;//
  private final int DEFAULT_MAX = 100;//倒计时时间(s)
  private final int DEFAULT_PROGRESS = 0;
  private final int DEFAULT_PROGRESS_COLOR = Color.parseColor("#FF7044");
  private final int DEFAULT_PROGRESS_BACK_COLOR = Color.parseColor("#00000000");
  ...
  private int max = DEFAULT_MAX;//倒计时时间(s)
  ...
  private void initView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    TypedArray attributes =
        context.obtainStyledAttributes(attrs, R.styleable.SegProgressbar, defStyleAttr, 0);
    mwidth = attributes.getDimension(R.styleable.SegProgressbar_p_width, width);
    mhight = attributes.getDimension(R.styleable.SegProgressbar_p_height, DEFAULT_HEIGTH);
    mShapeType = attributes.getInteger(R.styleable.SegProgressbar_p_shapeType, DEFAULT_SHAPE);
    showText = attributes.getBoolean(R.styleable.SegProgressbar_p_showText, false);
    showSegment = attributes.getBoolean(R.styleable.SegProgressbar_p_showProgressSegment, false);

    if (mShapeType == 1) {
      circleR =
          attributes.getDimension(R.styleable.SegProgressbar_p_circle_radius, DEFAULT_CIRCLE_R);
      //            showText = true;
      circleX = attributes.getDimension(R.styleable.SegProgressbar_p_circle_X_Y, circleR + 20);
    }
    if (mShapeType == 2) {
      squareRadius =
          attributes.getDimension(R.styleable.SegProgressbar_p_square_radius, DEFAULT_SQUARE_R);
    }

    if (showText) {
      textSize = attributes.getDimensionPixelSize(R.styleable.SegProgressbar_p_textSize,
          DEFAULT_TEXT_SIZE);
    }
    max = attributes.getInteger(R.styleable.SegProgressbar_p_maxValue, DEFAULT_MAX);
    progress = attributes.getInteger(R.styleable.SegProgressbar_p_progressValue, DEFAULT_PROGRESS);
    proColor =
        attributes.getColor(R.styleable.SegProgressbar_p_progressColor, DEFAULT_PROGRESS_COLOR);
    proBackColor = attributes.getColor(R.styleable.SegProgressbar_p_progressBackColor,
        DEFAULT_PROGRESS_BACK_COLOR);
    textLowColor = attributes.getColor(R.styleable.SegProgressbar_p_textLowColor, Color.BLACK);
    textMiddleColor = attributes.getColor(R.styleable.SegProgressbar_p_textMiddleColor, Color.BLUE);
    textHighColor = attributes.getColor(R.styleable.SegProgressbar_p_textHighColor, Color.RED);
    progressSegmentColor = attributes.getColor(R.styleable.SegProgressbar_p_progressSegmentColor,
        DEFAULT_PROGRESS_COLOR);
    progressDoubleSegColor =
        attributes.getColor(R.styleable.SegProgressbar_p_progressDoubleSegColor, Color.GRAY);
      ...
      case 0://rectangle
        ...
        if (progressList.size() > 0) {
          for (int i = 0; i < progressList.size(); i++) {
            canvas.drawLine(((float) progressList.get(i) / max) * mwidth, startY,
                ((float) progressList.get(i) / max) * mwidth, startY + (mhight - startY), paint);
          }
        }
        ...
        if (showSegment) {
          ...
          canvas.drawLine(((float) progress / max) * mwidth, startY,
              ((float) progress / max) * mwidth, startY + (mhight - startY), paint);
        }
        ...
        if (showText) {
          ...
          canvas.drawText((int) ((float) progress / max * DEFAULT_MAX) + "%",
              ((float) progress / max) * mwidth - (progress == 0 ? 0 : textSize), mhight + textSize,
              paint);
        }
        break;
      case 1://circle
        ...
        if (showText) {
          ...
          if (progress == max) {
            ...
            canvas.drawText((int) ((float) progress / max * DEFAULT_MAX) + "%",
                circleX + textSize / 2f, circleX + textSize * 2f, paint);
          }
        }
        break;
      case 2://square
